...
export default function ProjectCard({ item }: { item: ContentItem }) {
  ...
  // Prefer explicit external URL if provided in frontmatter
  const externalHref = item.displayUrl || item.link || (item.links && item.links.length > 0 ? item.links[0]?.href : undefined) || undefined;
